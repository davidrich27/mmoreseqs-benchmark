...
then
    ...
    echo "Usage: <i:raw_file> <benchmark> <file_type>"
    exit
fi
...
echo "RAW_TO_M3: $RAW_FILE => $M3_FILE"

# convert m3 file to tf file
bash ${SCRIPT_DIR}/m3_to_tf_pipeline.sh $M3_FILE $TF_FILE $BENCHMARK
echo "M3_TO_TF: $M3_FILE => $TF_FILE"

# downsample so results is in order of 100,000 entries
